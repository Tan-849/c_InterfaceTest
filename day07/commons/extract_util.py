...
from day07.commons.yml_util import YmlUtil
from day07.hotload.DebugTalk import DebugTalk
...
class ExtractUtil:
    ...
    # 解析传入的字典，把 $[csrf_token] 替换从 extract.yml 里面提取的具体的值
    def expr_replace_template(self, dict_data: dict):
        # 1. 字典转换为字符串
        data_str = yaml.safe_dump(dict_data, allow_unicode=True)
        # 2. 字符串替换
    ...
    # 解析传入的字典， ${func(args)} 通过反射去调用 func(args) 这个函数，实现只写 yml 然后调用 Python 代码
    def expr_replace_hotload(self, dict_data: dict):
        # 字典转换为字符串
        data_str = yaml.safe_dump(dict_data, allow_unicode=True)
        ...
        # regexp = "\\$\\{(.*?)\}"                # ${number}
        regexp = "\\$\\{(.*?)\\((.*?)\\)\\}"  # ${函数名(参数)}
        # 通过正则表达式在data str字符串中去匹配，得到所有的表达式list
